...
class Proof_Net(SimpleClipNet):
    def __init__(self, args, pretrained):
        ...
        self.common_match = get_attribute(args, "common_match", "common")
        ...
        if "nc" in self.setting:
            self.eft_head = ETFHead(self.num_classes, self.feature_dim,self._device,self.target_choose,self.target_match,self.common_match)
    ...
    def multiproto_proj(self,protos,proj):
        split_protos = [protos[:,i*self.feature_dim:(i+1)*self.feature_dim] for i in range(self.proto_num)]
        proj_protos = [proj(i) for i in split_protos]
        return torch.cat(proj_protos,dim=1)  #cls,1536
        # return torch.stack(proj_protos,dim=0).permute(1,0,2) # cls,3,512


    def extend_projection(self):
    ...
    def forward_transformer(self, image_features, text_features, transformer=True,prototype_features=None,use_multi_proto=True):
